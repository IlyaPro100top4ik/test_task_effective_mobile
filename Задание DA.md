...
- **мы принимаем готовое тестовое задание только в формате ссылки на GitHu**  
...
(  
    id integer, \-- ID счета  
    client\_id integer, \-- ID клиента  
    open\_dt date, \-- дата открытия счета  
    close\_dt date \-- дата закрытия счета  
)
...
(  
    ...
    transaction\_date date,  \-- дата транзакции  
